...
export function CodeExplanationModal({ 
  isOpen, 
  onClose, 
  automationId, 
  automationTitle 
  ...
  // Automatically generate explanation when modal opens
  useEffect(() => {
    if (isOpen && automationId && !explanation && !loading) {
      explainCode();
    }
  }, [isOpen, automationId]);
  ...
  const explainCode = async () => {
    if (!automationId) {
      toast.error("Error",'No automation ID provided');
      return;
    }

    setLoading(true);
    try {
      const response = await fetch(`/api/automations/${automationId}/explain-code`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        let errorMessage = 'Failed to explain code. Please try again.';
        try {
          const errorData = await response.json();
          errorMessage = errorData.error || errorMessage;
        } catch (e) {
          // If response is not JSON, use status text
          errorMessage = `Failed to explain code: ${response.statusText || 'Unknown error'}`;
        }
      ...
      toast.error("Error",'Failed to explain code. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Convert markdown-like explanation to HTML with proper code blocks
      ...
      <DialogContent className="max-w-5xl max-h-[85vh] overflow-hidden flex flex-col z-[9999] shadow-2xl">
        <DialogHeader className="pb-4 border-b">
          <DialogTitle className="flex items-center gap-3 text-xl font-bold text-color">
            ...
            Code Explanation
            {automationTitle && (
              <span className="text-base font-normal secondary-text">
                - {automationTitle}
              </span>
            )}
          </DialogTitle>
          <DialogDescription id="code-explanation-description" className="sr-only">
            AI-generated explanation of the automation code
          </DialogDescription>
        </DialogHeader>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center h-96 text-center">
                <Spin
                  ...
                  size="large"
                />
                <h3 className="text-2xl font-semibold mb-3 text-gray-800 dark:text-white">Generating Explanation...</h3>
                <p className="text-gray-600 dark:text-gray-400 text-lg">
                  AI is analyzing your automation code
                </p>
              ...
              <div className="flex flex-col items-center justify-center h-96 text-center">
                <FileText className="h-16 w-16 tertiary-text mb-6" />
                ...
                <p className="tertiary-text text-lg mb-6 max-w-md">
                  Click "Explain Code" to generate an AI-powered explanation of your automation.
                </p>
                <AntdButton 
                  onClick={explainCode} 
                  disabled={loading}
